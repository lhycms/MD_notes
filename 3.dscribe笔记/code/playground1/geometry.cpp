            ...
            double cutoff) 
{
    ...
    /*
        ...
        direction. We takes as many copies as needed to rearch the radial cutoff.
        Notice that we need to use vectors that are perpendicular to the cell vectors
        to ensure that the correct atoms are included for non-cubic cells.
    */
    auto position_u = positions.unchecked<2>();
    auto atomic_numbers_u = atomic_numbers.unchecked<1>();
    auto cell_u = cell.unchecked<2>();
    auto pbc_u = pbc.unchecked<1>();
    std::vector<double> a = {cell_u(0, 0), cell_u(0, 1), cell_u(0, 2)};
    std::vector<double> b = {cell_u(1, 0), cell_u(1, 1), cell_u(1, 2)};
    std::vector<double> c = {cell_u(2, 0), cell_u(2, 1), cell_u(2, 2)};

    std::vector<double> p1 = cross(b, c);
    std::vector<double> p2 = cross(c, a);
    std::vector<double> p3 = cross(a, b);

    // Projection of basis vectors onto perpenticular vectors
    ...
    std::vector<std::vector<double>> vectors = {p1, p2, p3};

    // Figure out how many copies to take per basis vector. Determine by
    // how many perpenticular projections fit into the cutoff distance.
    std::vector<int> n_copies_axis(3);
    std::vector<std::vector<int>> multipliers;
    for (int i=0; i<3; i++) {
        if (pbc_u(i)) {
            double length = norm(vectors[i]);
            double factor = cutoff / length;
            int multiplier = (int)std::ceil(factor);
            ...
            n_copies_axis[i] = 0;
            multipliers.push_back(std::vector<int>(0));
        }
    }
    ...
    for (int i=0; i<a_limit; i++) {
        ...
        for (int j=0; j<b_limit; j++) {
            ...
            for (int k=0; k<c_limit; k++) {
                ...
                // Precalculate the added vector.
                std::vector<double> addition(3, 0);
                for (int m=0; m<3; m++) {
                ...
                for (int l=0; l<n_atoms; l++) {
                    ...
                    for (int m=0; m<3; m++) {
                        ext_pos_mu(index, m) = position_u(l, m) + addition[m];
                    }
                }
                ++i_copy;
